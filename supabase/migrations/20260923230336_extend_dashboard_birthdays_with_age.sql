drop function if exists public.get_dashboard_upcoming_birthdays();

create function public.get_dashboard_upcoming_birthdays()
returns table (
  profile_id uuid,
  first_name text,
  last_name text,
...
as $$
  ...
  select
    ...
    w.days_until,
    case
      when w.days_until = 0 then
        extract(year from w.today)::integer
