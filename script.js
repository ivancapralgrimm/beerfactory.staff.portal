...
function initMenu(){
  ...
  // Optional live data can be provided by the host page without putting a
  // database token in this static bundle. Local data is always rendered first.
  const remoteConfig=globalThis.BEERFACTORY_MENU_CONFIG;
  ...
  async function table(config,id,forced){
    const controller=new AbortController();
    const timeout=setTimeout(()=>controller.abort(),5000);
    let r;
    try{
      const headers=config.token?{"xc-token":config.token}:{};
      const baseUrl=config.baseUrl.replace(/\/+$/,"");
      r=await fetch(`${baseUrl}/api/v2/tables/${id}/records?limit=1000`,{headers,signal:controller.signal});
    }finally{clearTimeout(timeout)}
    if(!r.ok)throw Error(r.status);
    const j=await r.json();
    const categoryField=config.categoryField||"Категория";
    const subcategoryField=config.subcategoryField||"Подкатегория";
    return (j.list||[]).map(x=>{
      const normalized=normalize(x[categoryField],x[subcategoryField],forced);
  ...
  /**
   * Парсит одну строку ингредиента вида "Название 123,5ед. примечание".
   * Название ищется как всё, что стоит ДО первого встреченного числа —
   * поэтому кавычки, скобки, дефисы, любые символы в названии не мешают
   * (например: Водка «Русский стандарт» 1л).
   * Всё, что идёт ПОСЛЕ единицы измерения (примечания вроде "(40%)"),
   * сохраняется без изменений и не отбрасывает всю строку, как раньше.
   */
  function parseIngredientLine(line){
    const match = line.match(/^(.+?)\s+(\d+(?:[.,]\d+)?)(\s*(?:мл|л|гр|г|кг|шт|порц|ст\.л|ч\.л|уп|капел[ьи])\.?)?(.*)$/i);
    if(!match) return null;
    const [, name, num, unitPart, rest] = match;
    return {
      name: name.trim(),
  ...
  function calc(text,m){
    return lines(text).map(line=>{
      ...
      if(!parsed) return line; // строка без числа (например "тоник — по вкусу") выводится как есть
      const scaled = formatScaledNumber(parsed.value*m);
      return `${parsed.name} ${scaled}${parsed.unitPart}${parsed.rest}`;
    });
  }
  ...
  render();
  if(remoteConfig?.baseUrl&&remoteConfig.tableId&&remoteConfig.kitchenTableId){
    Promise.all([
      table(remoteConfig,remoteConfig.tableId),
      table(remoteConfig,remoteConfig.kitchenTableId,"Кухня")
    ]).then(remoteItems=>{
      const merged=remoteItems.flat();
      if(merged.length){data=merged;render()}
    }).catch(e=>console.warn("Не удалось обновить меню, используются локальные данные",e));
  }
}
...
async function initTraining(){
  ...
  const state={category:"all",query:"",items:[],filtered:[],currentId:null};
  let progressFrame=0;
  ...
  renderLibrary();
  view?.addEventListener("scroll",requestReadingProgress,{passive:true});
  ...
  function requestReadingProgress(){
    ...
    progressFrame=requestAnimationFrame(()=>{
      progressFrame=0;
      if(!progressFill||!view)return;
      const max=view.scrollHeight-view.clientHeight;
      progressFill.style.width=max>0?`${Math.round(view.scrollTop/max*100)}%`:"0%";
    });
  }
  ...
  function card(item){
    ...
    el.innerHTML=`
      <button class="material-media" aria-label="${escapeHtml(item.title)}">
         <img src="${escapeAttr(item.cover||placeholderFor(item.category))}" alt="" loading="lazy">
    ...
    const fallback=placeholderFor(item.category);
    image.onerror=()=>{
      if(image.dataset.fallbackApplied)return image.remove();
  ...
  function openArticle(id){
    ...
    state.currentId=id;
    view.hidden=false; document.body.classList.add("article-open");
    const url=new URL(location.href);url.hash=encodeURIComponent(id);history.replaceState(null,"",url);
    view.scrollTo({top:0,behavior:"auto"});requestReadingProgress();
    ...
    main.innerHTML=`
      ...
      <p class="article-lead">${escapeHtml(item.description||"")}</p>
       ${item.cover?`<figure class="article-cover"><img src="${escapeAttr(item.cover)}" alt=""></figure>`:""}
  ...
  function renderSections(sections){
    return sections.map((s,i)=>{
      ...
      if(s.type==="list")return `<section class="article-section">${s.title?`<h2>${escapeHtml(s.title)}</h2>`:""}<ol>${(s.items||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ol></section>`;
  ...
  function buildToc(){
    const heads=[...$("#articleMain").querySelectorAll(".article-section h2,.article-callout b")].filter(h=>h.textContent.trim());
  ...
  function bindArticleImages(){
    $("#articleMain").querySelectorAll("img").forEach(img=>{
  ...
  function closeArticle(){view.hidden=true;document.body.classList.remove("article-open");state.currentId=null;if(progressFill)progressFill.style.width="0%";const url=new URL(location.href);url.hash="";history.replaceState(null,"",url)}
...
function initQuiz(){
  ...
  const startBtn=$("#start-btn"),backBtn=$("#back-to-training"),desc=$("#start-desc"),start=$("#start-screen"),quiz=$("#quiz-screen"),result=$("#result-screen"),qText=$("#question-text"),opts=$("#options-container"),progress=$("#question-progress"),next=$("#next-btn");
